...
This is a plain client for real WattTime data. Demo policy belongs in the carbon
provider layer and is never hidden inside this transport.
"""
...
class WattTimeClient:
    ...
    def _ensure_token(self, http_client: httpx.Client) -> None:
        ...
        login_response = http_client.get(
    ...
    def forecast_regions(self, client: httpx.Client | None = None) -> set[str]:
        """Return co2_moer regions for which this account has forecast access."""
        access = self.my_access(client=client)
        for signal in access.get("signal_types", []):
            if signal.get("signal_type") != "co2_moer":
                continue
            return {
                str(region["region"])
                ...
                and any(
                    endpoint.get("endpoint") == "v3/forecast"
                    for endpoint in region.get("endpoints", [])
                )
            }
        return set()
    ...
    def get_forecast(
        ...
        Missing credentials and upstream failures raise; this transport never invents data.
        """
        with client or httpx.Client() as http_client:
            response = self._request(
                "GET",
    ...
    def _assert_moer_units(self, payload: dict[str, Any]) -> None:
        ...
        if units is None and isinstance(payload.get("meta"), dict):
            units = payload["meta"].get("units")
